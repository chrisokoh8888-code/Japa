...
function addCourse() {
  let staffOptions = getStaff().map(s => `<option>${s}</option>`).join("");
  ...
  for (let c of data.courses) {
    for (let d of data.days) {
      for (let t of data.times) {
        ...
        timetable.push({ course:c.name, lecturer:c.lecturer, day:d, time:t, room:room.name });
...
function generate() {
  ...
  console.log(buildTimetable(data));
